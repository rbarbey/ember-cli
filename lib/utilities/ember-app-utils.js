...
*/
function normalizeUrl(rootURL, allowRelativeURL) {
  if (rootURL === undefined || rootURL === null || rootURL === '') {
    return '';
  }

  return cleanBaseURL(rootURL, allowRelativeURL);
}
...
function contentFor(config, match, type, options) {
  ...
  if ('rootURL' in config) {
    config.rootURL = normalizeUrl(config.rootURL, config.allowRelativeURL);
  }
...
function configReplacePatterns(options) {
  return [{
    match: /{{rootURL}}/g,
    replacement(config) {
      return normalizeUrl(config.rootURL, config.allowRelativeURL);
    },
  }, {
    match: /{{EMBER_ENV}}/g,
    replacement(config) {
